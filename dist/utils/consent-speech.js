import { getDetailTitle, getStaticTextEntries } from '../components/consent/consent-detail-view/consent-detail-view.logic';
...
export function buildListSpeechText(data) {
    const parts = [data.notice.title, data.notice.subtitle];
    for (const purpose of getVisiblePurposes(data.purposes)) {
        parts.push(purpose.name);
        if (purpose.badge)
            parts.push(purpose.badge);
...
export function buildDetailSpeechText(purpose, staticText) {
    const parts = [getDetailTitle(purpose)];
